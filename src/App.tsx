...
function App() {
	...
	const [roomId, setRoomId] = useState<number>(1); //not in use for now. for expansion.
	...
	const onValueFactored = () => {
		//factorisation to solve an issue where onvalue is not called by useEffect on first ever load.
		//This function is called on log-in if the bug is encountered.
		return onValue(ref(db, references.players(roomId)), (snapshot) => {
			...
			if (data) {
				// when players all quit , the database is empty. This handles the case when the user logs on with nobody on.
